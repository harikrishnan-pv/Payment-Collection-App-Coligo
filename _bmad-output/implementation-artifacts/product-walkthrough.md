...
*The "what is this product and why is it shaped this way" narrative, for the record. Companion to the PRD in `../planning-artifacts/prds/`.*
...
The scope is deliberately the **payment moment**, not a full banking app: no login, no loan origination, no statements. That matches the product's core loop (loan details + payment + acknowledgment + history) and keeps each included feature genuinely finished.

## The 60-second demo tour

1. Install the APK from the repo's Releases page (API URL baked in at build time — nothing to configure).
2. Enter demo account `ACC-100234` → loan details screen (all required fields).
...
The API landing page at `http://52.62.107.240/` documents every endpoint with copy-paste curls for quick manual testing.

## What shipped vs. what didn't

| Shipped (done) | Deliberately out (deferred) |
|---|---|
| Loan lookup by account number (all required fields) | Customer authentication / KYC |
...
1. **Mobile-only, one repo.** The product is a mobile app; API, app, and the shared contract package live in a single Turborepo workspace so types never drift.
...
6. **The API URL is an environment concern.** `EXPO_PUBLIC_API_URL` at build time, per the requirements; dev mode additionally auto-resolves to whichever machine serves the bundle.
